...
.favorite-div {
	...
	border-radius: 150px;
	background: #ffffff;
	box-shadow: 5px 5px 17px #e6e6e6, -5px -5px 17px #ffffff;
}

.favorite-div:hover {
	border-radius: 50%;
	background: linear-gradient(145deg, #e6e6e6, #ffffff);
	box-shadow: 10px 10px 26px #f0f0f0, -10px -10px 26px #ffffff;
}
...
.main-reg {
	...
	border-radius: 10px;
	background: #ffffff;
	box-shadow: 6px 6px 25px #e3e3e3, -6px -6px 25px #ffffff;
}
	...
	<c:choose>
		<c:when test="${not empty SearchResult }">
			...
			<table id="searchResult">
					...
					<tr>
						<td><strong>"${searchWord}" </strong> 키워드로 검색된 <strong>${totalRow}</strong>개의 상품이 있습니다.</td></tr>
				</table>
			<div class="list_main">
		...
		<c:otherwise>
				...
				<table id="searchResult">
					...
					<tr>
						<td><strong>"${searchWord}" </strong> 키워드로 검색된 <strong>${totalRow}</strong>개의 상품이 있습니다.</td></tr>
							<tr><td>죄송합니다. 빠른 시일 내로 상품을 준비하겠습니다.</td></tr>
					
				</table>
			 <img src="${root }images/termsback.jpg"; style="width: 1000px; margin: auto; display: block;">
		</c:otherwise>
	</c:choose>
